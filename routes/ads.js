...
function normalizeForFrontend(ad) {
  return {
    ...
    impressions:             null,
    currency:                'USD',
    ad_snapshot_url:         ad.image        || ad.snapshot_url || null,
    image:                   ad.image        || null,
  ...
  try {
    ...
    if (MetaAd && mongoose.connection.readyState === 1) {
      ...
      const [ads, total] = await Promise.all([
        MetaAd.find(query)
          .sort({ priority: -1, featured: -1, scraped_at: -1 })
          .skip(skip)
          .limit(parseInt(limit))
          .lean(),
        MetaAd.countDocuments(query),
      ]);

      if (ads.length > 0) {
        const normalized = ads.map(normalizeForFrontend);
        console.log('[Meta Route] MongoDB se serve: ' + normalized.length + ' ads');
...
// GET /api/ads/image-proxy — Facebook image fresh fetch karke serve karo
// Kabhi expire nahi hogi — backend fresh URL fetch karta hai
router.get('/image-proxy', async (req, res) => {
  ...
  try {
    ...
    // Agar URL nahi hai toh library ID se MongoDB se lo
    if (!imageUrl && id && MetaAd && mongoose.connection.readyState === 1) {
      const ad = await MetaAd.findOne({ library_id: id }).select('image snapshot_url').lean();
    ...
    // Facebook CDN se image fetch karo
    ...
    // Cache headers — 24 ghante cache karo browser mein
    ...
    // Agar image fetch fail ho toh 1x1 transparent pixel return karo
