...
function normalizeForFrontend(ad) {
  return {
    ...
    impressions:             null,
    currency:                'USD',
    ad_snapshot_url:         ad.r2_image_url || ad.image || ad.snapshot_url || null,
    image:                   ad.r2_image_url || ad.image || null,
    r2_image_url:            ad.r2_image_url || null,
    video_url:               ad.r2_video_url || ad.video || null,   // R2 first (permanent)
    r2_video_url:            ad.r2_video_url || null,               // Permanent R2 URL
    video:                   ad.r2_video_url || ad.video || null,
...
router.get('/meta', protect, async (req, res) => {
  ...
  try {
    ...
    if (MetaAd && mongoose.connection.readyState === 1) {
      ...
      // Duplicate brands hataane ke liye aggregate use karo
      const pipeline = [
        { $match: query },
        { $sort: { priority: -1, featured: -1, scraped_at: -1 } },
        // Same brand ka sirf ek ad dikhao (best one)
        { $group: {
          _id: '$brand',
          doc: { $first: '$$ROOT' }
        }},
