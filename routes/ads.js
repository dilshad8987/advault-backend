...
router.get('/meta', protect, async (req, res) => {
  ...
  try {
    ...
    console.log('[Meta Route] result type:', typeof result);
    console.log('[Meta Route] result keys:', Object.keys(result || {}));

    // facebook-scraper-api4 returns different shapes — handle all cases:
    // { ads: [...] }
    // { data: { ads: [...] } }
    // { data: [...] }
    // { results: [...] }
    // { ad_archive_ids: [...] }  ← some endpoints return ids only
    // direct array
    let raw = [];
    ...
    if (Array.isArray(result)) {
      ...
      // Last resort — find any array in top-level
      const arr = Object.values(result).find(v => Array.isArray(v) && v.length > 0);
      if (arr) raw = arr;
    }

    console.log('[Meta Route] Normalized ad count:', raw.length);
    if (raw.length > 0) console.log('[Meta Route] First ad keys:', Object.keys(raw[0] || {}));

    res.json({ success: true, data: raw, total: raw.length });
  } catch (err) {
    console.error('[Meta Route] Error:', err.response?.status, err.response?.data || err.message);
    if (err.response?.status === 429)
      return res.status(429).json({ success: false, message: 'Rate limit — thodi der baad try karo' });
