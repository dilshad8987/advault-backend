...
// ─── Rate limit queue — ek waqt mein sirf 1 API call ─────────────────────────
let _queue = Promise.resolve();
let _lastCall = 0;
const MIN_INTERVAL = 1200; // 1.2s min between calls
...
// ─── Retry wrapper — 429 pe wait karke retry karo ─────────────────────────────
  ...
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      if (status === 429 && i < retries) {
        console.log(`⏳ Rate limited (429) — ${delayMs}ms baad retry (attempt ${i+1}/${retries})`);
...
async function searchTikTokAds({ country = 'US', order = 'like', keyword = '', period = '7' }) {
  const cacheKey = makeCacheKey('tiktok_search', { country, order: order || 'like', keyword, period });
...
async function getTopProducts({
  ...
  return getOrFetch(cacheKey, async () => {
    return rateLimitedCall(() => withRetry(async () => {
      const params = {
        page, limit, country_code: country, ecom_type: ecomType,
        order_by: orderBy, order_type: orderType, period_type: periodType, last,
      };
...
async function getTikTokVideoInfo(tiktokUrl) {
  ...
  return getOrFetch(cacheKey, async () => {
    return rateLimitedCall(() => withRetry(async () => {
      const res = await client.get('/', { params: { url: tiktokUrl, hd: 1 } });
      return res.data;
    }));
  }, 3600);
}
...
module.exports = {
  ...
  getTopProducts, getProductDetail, getTikTokVideoInfo,
